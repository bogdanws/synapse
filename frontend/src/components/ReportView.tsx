...
import { AppNavbar, SynapseBrandLink } from './AppNavbar'
...
interface ReportViewProps {
   ...
   * that does not exist on the backend; a conversion CTA is shown instead.
   */
  sample?: boolean
}
  ...
  return (
    ...
    >
      ...
      <AppNavbar variant="app" className="report-nav px-7">
        <div className="report-nav-meta">
          ...
          <span className="micro report-nav-brief">
            {sample ? 'Sample report' : `Brief #${jobId.slice(0, 8).toUpperCase()}`} · Delivered{' '}
        ...
        <div className="report-nav-actions">
          {sample ? (
            <Link to="/register">
              <Button size="sm">Start your own brief →</Button>
            </Link>
          ) : (
            <>
              <Button
      ...
      <div style={{ flex: 1 }}>
        ...
        {/* Contradictions section — rendered only when Scribe identified at least one */}
          ...
          >
            <div className="report-main-cell">
              <section
                ...
                style={{ paddingTop: 24, borderTop: '1px solid var(--fg)' }}
              >
                <div className="micro" style={{ marginBottom: 16 }}>
                  Where sources disagree · {report.contradictions.length}{' '}
                  {report.contradictions.length === 1 ? 'contradiction' : 'contradictions'} found
                </div>
                <ContradictionsList
